...
This module is intentionally not imported by the normal AliBot runtime. It is
used only by the isolated Railway evidence-test service. It initializes the
same extraction composition as entrypoint.py, then repeatedly submits a small
set of public test URLs through the real extractor so the shadow observer can
collect paired evidence without Telegram polling or user traffic.
"""
...
async def main() -> None:
    ...
    print("🧪 Paired Evidence Harness: sampling remains controlled by collector policy.", flush=True)
    ...
    interval = max(5.0, float(os.getenv("ALIBOT_EVIDENCE_HARNESS_INTERVAL", "30")))
    ...
    while rounds == 0 or completed < rounds:
        for source_url in TEST_URLS:
            ...
            try:
                ...
                print(
                    f"🧪 Paired Evidence Harness: extraction completed candidates={count} "
                    f"elapsed_ms={(time.monotonic() - started) * 1000:.0f}",
                    flush=True,
                )
            except Exception as exc:
                print(
                    f"⚠️ Paired Evidence Harness: extraction failed {type(exc).__name__}",
                    flush=True,
                )
        completed += 1
