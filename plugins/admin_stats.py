...
def _stats(get_db) -> dict[str, Any]:
    ...
    try:
        ...
        videos = conn.execute(
            "SELECT COUNT(*) AS count FROM downloads WHERE media_type = 'video'"
        ).fetchone()["count"]
        audio = conn.execute(
            "SELECT COUNT(*) AS count FROM downloads WHERE media_type = 'audio'"
        ).fetchone()["count"]
        phones = conn.execute(
            "SELECT COUNT(*) AS count FROM users WHERE phone IS NOT NULL AND phone != ''"
        ).fetchone()["count"]
        ...
        genders = conn.execute(
            "SELECT gender, COUNT(*) AS count FROM users WHERE gender IS NOT NULL AND gender != '' GROUP BY gender"
        ).fetchall()
        return {
            ...
            "audio": int(audio or 0),
            "phones": int(phones or 0),
...
def _keyboard(days: int = 30) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("1 يوم", callback_data="admin_dashboard_1"),
            InlineKeyboardButton("7 أيام", callback_data="admin_dashboard_7"),
            InlineKeyboardButton("30 يوم", callback_data="admin_dashboard_30"),
        ],
        ...
        [
            ...
            InlineKeyboardButton("🌐 أكثر المنصات", callback_data="admin_top_websites"),
        ],
...
async def stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, get_db, owner_id: int) -> None:
    ...
    lines = [
        "📈 <b>الإحصائيات</b>", "━━━━━━━━━━━━━━━━━━━━", "",
        f"👥 المستخدمون: {data['users']}", f"🚫 المحظورون: {data['banned']}",
        f"📥 التحميلات: {data['downloads']}", f"🎥 الفيديوهات: {data['videos']}",
        f"🎵 الصوتيات: {data['audio']}", f"📱 الهواتف: {data['phones']}", f"📍 المواقع: {data['locations']}",
        "", "🚻 <b>الجنس</b>",
    ]
    ...
    await query.edit_message_text("\n".join(lines)[:3900], parse_mode="HTML", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("📊 لوحة المعلومات", callback_data="admin_dashboard_30")],
        [InlineKeyboardButton("🎛️ مركز التحكم", callback_data="admin_control_center")],
    ]))
...
async def recent_downloads_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, get_db, owner_id: int) -> None:
    ...
    try:
        rows = conn.execute(
            "SELECT user_id, website, media_type, quality, created_at FROM downloads ORDER BY id DESC LIMIT 15"
        ).fetchall()
    finally:
        conn.close()
    ...
    await query.edit_message_text("\n".join(lines)[:3900], parse_mode="HTML", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 تحديث", callback_data="admin_recent_downloads")],
        [InlineKeyboardButton("📊 لوحة المعلومات", callback_data="admin_dashboard_30")],
    ]))
...
async def top_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, get_db, owner_id: int) -> None:
    ...
    try:
        rows = conn.execute(
            "SELECT user_id, username, first_name, downloads FROM users ORDER BY downloads DESC, last_seen DESC LIMIT 15"
        ).fetchall()
    finally:
        conn.close()
