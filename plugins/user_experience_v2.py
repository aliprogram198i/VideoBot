...
def _set_preferences(bot_module: Any, user_id: int, media_type: str, quality: str) -> None:
    ...
    try:
        conn.execute(
            """INSERT INTO user_preferences(user_id, media_type, quality, updated_at) VALUES (?, ?, ?, ?)
...
def _quality_keyboard(media_type: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton(label, callback_data=f"ux_quality_{value}")] for value, label in labels]
        + [[InlineKeyboardButton("🔙 الإعدادات", callback_data="ux_settings")]]
    )
...
async def _library_screen(target, bot_module: Any, user_id: int) -> None:
    ...
    try:
        rows = conn.execute(
            "SELECT id, website, title FROM user_favorites WHERE user_id = ? ORDER BY id DESC LIMIT 10",
            (user_id,),
        ).fetchall()
    finally:
        conn.close()
        ...
        try:
            conn.execute(
                """INSERT OR IGNORE INTO user_favorites(user_id,url,website,media_type,quality,title,created_at)
                   VALUES (?,?,?,?,?,?,?)""",
                (user.id, url, info.get("source"), "video", "", info.get("title"), _now()),
            )
            conn.commit()
        finally:
            conn.close()
