...
legacy download pipeline is allowed to continue. The existing quality and
upload pipeline remains unchanged after verification.
"""
...
def parse_telegram_message_url(url: str) -> TelegramMessageRef | None:
    ...
    if not match or match.group("channel").lower() == "c":
        return None
...
def _metadata_matches(ref: TelegramMessageRef | None, info: dict) -> bool:
    ...
    if ref.is_internal_channel:
        ...
        if channel_id:
            ...
            if normalized not in {ref.channel, f"100{ref.channel}"}:
                return False
    else:
