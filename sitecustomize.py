...
on ``sys.path``.  We use that hook only to install the already-tested
``admin_layer_v2`` immediately before the existing bot starts polling.  This
avoids rewriting the large legacy ``bot.py`` registration block in-place while
ensuring the canonical layer removes duplicate legacy admin handlers first.
"""
