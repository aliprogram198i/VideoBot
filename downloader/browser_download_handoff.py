...
DEFAULT_MIN_VIDEO_DURATION = 0.0
...
def _safe_filename(name: str, is_audio: bool) -> str:
    ...
    if not clean or clean in {".", ".."}:
        clean = "audio" if is_audio else "video"
    return clean[:180]
...
def _verify_media_file(path: str, *, is_audio: bool, min_bytes: int, min_duration: float, max_file_bytes: int = DEFAULT_MAX_FILE_BYTES) -> bool:
    ...
    try:
        ...
        if completed.returncode != 0:
            print("⚠️ Browser Download Handoff: ffprobe rejected file", flush=True)
            return False
        duration = float((completed.stdout or "").strip())
        if duration < min_duration:
            print(f"⚠️ Browser Download Handoff: video duration rejected ({duration:.2f}s)", flush=True)
            return False
        return True
    except (ValueError, OSError, subprocess.SubprocessError):
        print("⚠️ Browser Download Handoff: media verification failed", flush=True)
        return False
...
async def _candidate_links(page, page_url: str) -> list[str]:
    ...
    for row in rows or []:
        ...
        if _is_download_target(text, link):
            score += 30
        if _looks_like_media(link):
            score += 20
        if score >= 10:
...
def _stream_with_browser_context(url: str, output_dir: str, *, cookies: list[dict], referer_url: str | None, is_audio: bool, max_file_bytes: int, timeout_s: float, min_bytes: int = DEFAULT_MIN_VIDEO_BYTES, min_duration: float = DEFAULT_MIN_VIDEO_DURATION) -> str | None:
...
async def _click_controls(page, max_clicks: int) -> None:
    ...
    for row in rows or []:
        ...
        if _is_download_target(text, href):
            score += 30
        if score >= 14:
...
async def _save_async(candidate_url: str, output_dir: str, *, validator, is_audio: bool, timeout_ms: int, settle_ms: int, max_file_bytes: int, referer_url: str | None = None, min_video_bytes: int = DEFAULT_MIN_VIDEO_BYTES, min_video_duration: float = DEFAULT_MIN_VIDEO_DURATION) -> str | None:
                ...
                try:
                    ...
                    for download in download_holder:
                        try:
                            ...
                            if suffix.lower() not in _MEDIA_FILE_EXTENSIONS:
                                suffix = ".m4a" if is_audio else ".mp4"
                            fd, target = tempfile.mkstemp(prefix="browser_", suffix=suffix, dir=output_dir)
                            os.close(fd)
                            try:
...
def resolve_to_file(candidate_url: str, output_dir: str, *, validator, is_audio: bool = False, timeout_ms: int = DEFAULT_TIMEOUT_MS, settle_ms: int = DEFAULT_SETTLE_MS, max_file_bytes: int = DEFAULT_MAX_FILE_BYTES, referer_url: str | None = None, min_video_bytes: int = DEFAULT_MIN_VIDEO_BYTES, min_video_duration: float = DEFAULT_MIN_VIDEO_DURATION) -> str | None:
    """Resolve a public browser candidate into a verified local file."""
    try:
        return asyncio.run(_save_async(candidate_url, output_dir, validator=validator, is_audio=is_audio, timeout_ms=timeout_ms, settle_ms=settle_ms, max_file_bytes=max_file_bytes, referer_url=referer_url, min_video_bytes=min_video_bytes, min_video_duration=min_video_duration))
    except RuntimeError:
        loop = asyncio.new_event_loop()
        try:
            return loop.run_until_complete(_save_async(candidate_url, output_dir, validator=validator, is_audio=is_audio, timeout_ms=timeout_ms, settle_ms=settle_ms, max_file_bytes=max_file_bytes, referer_url=referer_url, min_video_bytes=min_video_bytes, min_video_duration=min_video_duration))
        finally:
            loop.close()
    except Exception as exc:
