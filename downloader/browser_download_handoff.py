...
_MEDIA_FILE_EXTENSIONS = (".mp4", ".m4v", ".webm", ".mov", ".mkv", ".avi", ".flv", ".wmv", ".mp3", ".m4a", ".aac", ".wav", ".ogg", ".opus", ".flac")
...
def _is_media_response(url: str, content_type: str | None, disposition: str | None) -> bool:
    ct = (content_type or "").lower(); cd = (disposition or "").lower()
...
def _score_control(text: str, href: str) -> int:
    hay = f"{text} {href}".lower(); score = 0
    for token, weight in (("download", 8), ("تحميل", 8), ("server", 5), ("سيرفر", 5), ("player", 4), ("مشاهدة", 3), ("watch", 3), ("play", 2)):
...
async def _candidate_links(page, page_url: str) -> list[str]:
    try:
        rows = await page.locator("a[href], iframe[src], embed[src]").evaluate_all("""els => els.map((el,index) => ({index, href: el.href || el.src || '', text: (el.innerText || el.textContent || '').trim(), attr: ((el.className || '') + ' ' + (el.id || '')).trim()}))""")
    except Exception:
        return []
    base = urlparse(page_url); ranked = []
    for row in rows or []:
        if not isinstance(row, dict): continue
        link = str(row.get("href") or "")
        if not _is_http_url(link): continue
        parsed = urlparse(link); text = str(row.get("text") or "")
        score = 10 if parsed.netloc == base.netloc else 2
        score += _score_control(text, link)
        if _is_download_target(text, link): score += 30
...
def _stream_with_browser_context(url: str, output_dir: str, *, cookies: list[dict], referer_url: str | None, is_audio: bool, max_file_bytes: int, timeout_s: float) -> str | None:
    ...
    try:
        with urlopen(Request(url, headers=headers), timeout=timeout_s) as response, open(target, "wb") as output:
            total = 0; content_length = response.headers.get("Content-Length")
            if content_length:
                try:
            ...
            while True:
                ...
                output.write(chunk)
        if total > 0:
            print(f"🌐 Browser Download Handoff: streamed {total} bytes", flush=True); return target
...
async def _click_controls(page, max_clicks: int) -> None:
    ...
    try:
        rows = await page.locator(selector).evaluate_all("""els => els.map((el,index) => ({index, href: el.href || '', text: (el.innerText || el.textContent || el.value || '').trim(), attr: ((el.className || '') + ' ' + (el.id || '') + ' ' + (el.getAttribute('data-server') || '') + ' ' + (el.getAttribute('data-player') || '') + ' ' + (el.getAttribute('data-download') || '')).trim()}))""")
    ...
    for row in rows or []:
        ...
        href = str(row.get("href") or ""); text = f"{row.get('text') or ''} {row.get('attr') or ''}"
        score = _navigation_score(text, href)
        if _is_download_target(text, href): score += 30
        if score >= 14: ranked.append((score, index))
    ranked.sort(key=lambda item: (-item[0], item[1])); locator = page.locator(selector)
    for _, index in ranked[:max_clicks]:
        try:
            await locator.nth(index).click(timeout=1800, no_wait_after=True); await page.wait_for_timeout(500)
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if referer_url:
                ...
                try:
                    await source_page.goto(referer_url, wait_until="domcontentloaded", timeout=timeout_ms); await source_page.wait_for_timeout(settle_ms)
                except Exception: pass
                finally: await source_page.close()
            queue = [candidate_url]; visited: set[str] = set()
            while queue and len(visited) < DEFAULT_MAX_PAGES:
                page_url = queue.pop(0)
                if page_url in visited: continue
                visited.add(page_url); print(f"🌐 Browser Download Handoff: opening page {len(visited)}/{DEFAULT_MAX_PAGES}", flush=True)
                page = await context.new_page(); media_urls: list[str] = []; seen_media: set[str] = set(); download_holder: list[object] = []
                def remember_media(url: str) -> None:
                    if _is_http_url(url) and url not in seen_media and len(media_urls) < DEFAULT_MAX_MEDIA_RESPONSES: seen_media.add(url); media_urls.append(url)
                async def on_response(response) -> None:
                    try: headers = response.headers; content_type = headers.get("content-type"); disposition = headers.get("content-disposition"); response_url = response.url
                    except Exception: return
                    if _is_media_response(response_url, content_type, disposition):
                        try: validator(response_url)
                        except Exception: return
                        remember_media(response_url); print("🌐 Browser Download Handoff: captured media response", flush=True)
                async def on_download(download) -> None:
                    ...
                    download_holder.append(download); remember_media(download_url); print("🌐 Browser Download Handoff: captured browser download", flush=True)
                ...
                try:
                    try:
                        ...
                        # A direct download navigation is expected to raise here. Do not
                        # skip the captured Download object or captured media responses.
                        print(f"⚠️ Browser Download Handoff: navigation failed ({type(exc).__name__}); continuing with captured media", flush=True)
                    await page.wait_for_timeout(settle_ms); await _click_controls(page, DEFAULT_MAX_CLICKS); await page.wait_for_timeout(settle_ms)
                    try: dom_rows = await page.locator("video, audio, source").evaluate_all("""els => els.map(el => el.currentSrc || el.src || el.getAttribute('src') || el.getAttribute('data-src') || el.getAttribute('data-url') || '')""")
                    ...
                    for download in download_holder:
                        try:
                            suggested = _safe_filename(download.suggested_filename or ("audio" if is_audio else "video"), is_audio); suffix = Path(suggested).suffix
                            if suffix.lower() not in _MEDIA_FILE_EXTENSIONS: suffix = ".m4a" if is_audio else ".mp4"
                            fd, target = tempfile.mkstemp(prefix="browser_", suffix=suffix, dir=output_dir); os.close(fd)
                            try:
                                await download.save_as(target); size = os.path.getsize(target)
                                if 0 < size <= max_file_bytes:
                                    print(f"🌐 Browser Download Handoff: saved {size} bytes", flush=True); return target
                            ...
                            try:
                                ...
                                if path and os.path.isfile(path):
                                    ...
                                    if 0 < size <= max_file_bytes:
                                        ...
                                        if suffix not in _MEDIA_FILE_EXTENSIONS: suffix = ".m4a" if is_audio else ".mp4"
                                        target = tempfile.mktemp(prefix="browser_", suffix=suffix, dir=output_dir); shutil.copyfile(path, target)
                                        if os.path.getsize(target) == size:
                                            print(f"🌐 Browser Download Handoff: recovered download path ({size} bytes)", flush=True); return target
                            except Exception as path_exc:
                                print(f"⚠️ Browser Download Handoff: download path unavailable ({type(path_exc).__name__})", flush=True)
                    cookies = await context.cookies()
                    if media_urls: print(f"🌐 Browser Download Handoff: streaming {len(media_urls)} captured media URL(s)", flush=True)
                    ...
                    print(f"⚠️ Browser Download Handoff: page processing failed ({type(exc).__name__})", flush=True)
...
def resolve_to_file(candidate_url: str, output_dir: str, *, validator, is_audio: bool = False, timeout_ms: int = DEFAULT_TIMEOUT_MS, settle_ms: int = DEFAULT_SETTLE_MS, max_file_bytes: int = DEFAULT_MAX_FILE_BYTES, referer_url: str | None = None) -> str | None:
    ...
    try:
        ...
        LOG.warning("Browser download handoff failed: %s", type(exc).__name__); return None
