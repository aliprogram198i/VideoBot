...
_MEDIA_MARKERS = (".m3u8", ".mpd", ".mp4", ".m4v", ".webm", ".mov", ".avi", ".mkv", ".ts", ".m4a", ".mp3", ".aac")
_MEDIA_FILE_EXTENSIONS = {".mp4", ".m4v", ".webm", ".mov", ".avi", ".mkv", ".ts", ".m3u8", ".mpd", ".m4a", ".mp3", ".aac"}
_SERVER_WORDS = ("server", "servers", "watch", "player", "stream", "source", "سيرفر", "سيرفرات", "مشاهدة", "مشغل", "مشاهده", "تشغيل", "السيرفر", "السيرفرات")
_DOWNLOAD_WORDS = ("download", "downloads", "direct", "تحميل", "تحميل مباشر", "تنزيل", "رابط التحميل", "hd", "web-dl", "webrip")
...
def _is_media_response(url: str, content_type: str | None, content_disposition: str | None = None) -> bool:
    ...
    if "attachment" in disposition:
        ...
        filename = ((filename_match.group(1) if filename_match and filename_match.group(1) else "") or (filename_match.group(2) if filename_match and filename_match.group(2) else "")).lower()
...
def _score(url: str, content_type: str | None) -> int:
    ...
    path = urlparse(url).path.lower()
    if "mpegurl" in normalized or ".m3u8" in path: return 120
    if "dash+xml" in normalized or ".mpd" in path: return 115
    if normalized.startswith("video/"): return 110
    if normalized.startswith("audio/"): return 105
    return 100
...
async def _discover_navigation_targets(page, base_url: str, max_targets: int) -> list[str]:
    try:
        rows = await page.locator("a[href], iframe[src], embed[src]").evaluate_all("""els => els.map(el => ({href: el.href || el.src || '', text: (el.innerText || el.textContent || '').trim(), attr: ((el.className || '') + ' ' + (el.id || '') + ' ' + (el.getAttribute('data-server') || '') + ' ' + (el.getAttribute('data-player') || '') + ' ' + (el.getAttribute('data-download') || '')).trim()}))""")
    ...
    for row in rows or []:
        ...
        href = row.get("href")
        if not isinstance(href, str) or not _is_http_url(href) or href == base_url: continue
        ...
        if download_target: score += 20
        if score <= 0: continue
        current = ranked.get(href)
        candidate = (score, text, download_target)
        if current is None or score > current[0]: ranked[href] = candidate
    ordered = sorted(ranked.items(), key=lambda item: (-int(item[1][2]), -item[1][0], item[0]))
...
async def _collect_dom_media(page, validator, candidates: dict[str, tuple[int, str | None]]) -> None:
    ...
    for row in rows or []:
        ...
        if not isinstance(media_url, str) or not _is_http_url(media_url): continue
        ...
        try:
            ...
            while queue and len(visited_pages) < max_pages:
                ...
                async def on_response(response) -> None:
                    ...
                    if responses_seen >= DEFAULT_MAX_RESPONSES: return
                    responses_seen += 1
                    response_url = response.url
                    if not _is_http_url(response_url): return
                    try:
                ...
                async def on_download(download) -> None:
                    ...
                    path, query = parsed.path.lower(), parsed.query.lower()
                    is_media_file = any(path.endswith(ext) for ext in _MEDIA_FILE_EXTENSIONS) or any(suggested.endswith(ext) for ext in _MEDIA_FILE_EXTENSIONS)
                    secure_media_endpoint = any(marker in query or marker in path for marker in ("secure_stream", "download", "direct_stream", "media"))
                    if not (is_media_file or secure_media_endpoint): return
