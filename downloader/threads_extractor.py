...
def _add(result: list[ThreadsMedia], seen: set[tuple[str, str]], url: str, kind: str, source: str, confidence: int) -> None:
    key = (url, kind)
    if key in seen:
        return
    seen.add(key)
    result.append(ThreadsMedia(url=url, kind=kind, discovered_by=source, confidence=confidence))


def extract_threads_media(page: str, page_url: str, *, max_candidates: int = 50) -> list[ThreadsMedia]:
    ...
    key_pattern = re.compile(
        ...
        r"video_src|videoSrc|progressive_url|progressiveUrl)[A-Za-z0-9_]*)[\"']?"
        r"\s*[:=]\s*[\"']([^\"']+)[\"']",
        re.IGNORECASE,
    )
    ...
    og_pattern = re.compile(
        r"<meta\b[^>]*?(?:property|name)\s*=\s*[\"']og:video(?::url)?[\"']"
        r"[^>]*?content\s*=\s*[\"']([^\"']+)[\"'][^>]*>",
        re.IGNORECASE,
    )
    for match in og_pattern.finditer(page):
        url = _absolute(match.group(1), page_url)
        if url:
