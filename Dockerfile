...
RUN test -s /opt/yt-dlp-plugins/yt_dlp_plugins/extractor/threads.py \
    && python -m py_compile bot.py entrypoint.py stats_entrypoint.py downloader/error_reporter.py downloader/smart_search.py downloader/smart_media_bridge.py downloader/smart_media_resolver.py downloader/browser_media_resolver.py downloader/browser_download_handoff.py downloader/shahid4u_resolver.py downloader/smart_learning_foundation.py jobs/download_manager.py security/download_guard.py security/rate_limit.py plugins/manager.py plugins/core_runtime.py plugins/admin_common.py plugins/admin_control_center.py plugins/admin_layer_v2.py plugins/admin_operations_center.py plugins/admin_fallback_intelligence.py plugins/admin_backup_recovery.py plugins/admin_security_center.py plugins/admin_smart_analytics.py plugins/admin_image_ai.py plugins/admin_user_history.py plugins/admin_global_history.py plugins/smart_operations.py plugins/recovered_features.py plugins/smart_search_pro.py plugins/user_features.py plugins/yoinku_compat.py plugins/whatsapp_audio.py telegram_layer/media_retry.py
RUN python -c "import os,tempfile,shutil; from plugins import admin_image_ai as m; d=tempfile.mkdtemp(prefix='image-ai-smoke-'); os.environ['IMAGE_AI_DATA_DIR']=d; m._ensure_schema(); m._save_instruction('smoke instruction'); m._save_example('source','result','smoke edit'); assert m._count('image_ai_instructions','active=1') == 1; assert m._count('image_ai_examples',m._DATASET_WHERE) == 1; shutil.rmtree(d)"
